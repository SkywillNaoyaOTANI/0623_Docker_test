...
cd /var/www/my-docker-web
mkdir -p compose-app/web/maintenance compose-app/db/init
cd compose-app


vim docker-compose.yml
vim db/init/01_init.sql
vim web/requirements.txt
vim web/app.py
vim web/check_db.py
vim web/entrypoint.sh
vim web/maintenance/index.html
vim web/Dockerfile


# docker-compose.ymlに書かれた全部のコンテナをまとめて起動するコマンド
docker compose up -d
...
# Webコンテナを再起動する（この起動時にDBへ通信を試みます）
